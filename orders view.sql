...
CREATE VIEW orders_report AS
  SELECT o.order_number, o.id order_id, c.email, c.address, c.last_name || ' ' || c.first_name || ' ' || c.middle_name full_name, s.name status, o.completed_at completed_at, op.price, op.quantity, p.id product_id, p.name product, pt.name product_type, p.price current_price, o.status_id
